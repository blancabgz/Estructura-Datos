...
Cola<T>::~Cola(){
  Celda * aux;
  while (primera!=nullptr){             //Mientras queden elementos
...
Cola<T>& Cola<T>::operator=(const Cola<T> & otra){
  ...
  if (this != &otra){    //Comprobación de rigor. Si son diferentes objetos
    ...
    else primera=ultima=0;      //Si la otra cola esta vacia
...
void Cola<T>::poner(const T & elem){
  ...
  if (primera==0)                    //Si la lista esta vacia,
...
void Cola<T>::quitar(){
  ...
  num_elem--;                     //Actualizamos el numero de elementos
}
